...
// Dot product
double Coord3::dotProduct(Coord3 ope){
	return (x*ope.x) + (x*ope.y) + (z*ope.z);
}
...
// Calulate cross prodcut
Coord3 Coord3::crossProduct(Coord3 v){
	
	// Perform the cross product
	double n1 = (y * v.z) - (z - v.y);
  double n2 = (z * v.x) - (x - v.z);
  double n3 = (x * v.y) - (y - v.x);

	// Return a new coord
	return Coord3(n1,n2,n3);
}