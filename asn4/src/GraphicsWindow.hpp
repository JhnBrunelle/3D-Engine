...
public:
	...
	/**
	 * Draws the points from the scene, and creates lines
	 *
	 */
    void drawScene();

	/**
	 * Shades in a Fcce aka PolyFill, with the correct color
	 * PolyFill &(in) - Reference to a PolyFill
	 * color (in) - unsigned long color to fill with
	 */
	void shadeFace(PolyFill &pf, unsigned long color);
...
private:
	 ...
	 * returns void
	 */
	void Bresenham(int x1, int y1, int x2, int y2,  unsigned long colour);
	 ...
	 * returns void
	 */
	void drawLine(int x1, int y1, int x2, int y2, int type, unsigned long colour);
