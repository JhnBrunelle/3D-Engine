...
void GraphicsWindow::drawScene(){
  ...
  // Create a new Scene
  Scene projected = camera.createScene(scene);

  // Iterate through each shape 
  for(int i = 0; i < projected.getShapes().size(); i++){

  	// Iterate through each line in the shape
    Shape newShape = projected.getShapes()[i];
    for(int j = 0; j < newShape.getLines().size(); j++){

      // Draw the line
      Line l = newShape.getLines()[j];
      Bresenham((int)(l.a.x), (int)(l.b.x), (int)(l.a.y), (int)(l.b.y));
    }
  }
}
